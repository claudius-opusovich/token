...
import { eventWithShortcode, factoryEventSentBy, getMxIdLocalPart, guessDmRoomUserId } from '../../utils/matrix';
...
import { roomWallpapersAtom } from '../../state/roomWallpapers';
...
export function RoomTimeline({ room, eventId, roomInputRef, editor }: RoomTimelineProps) {
  ...
  const [dmReceiptEventId, setDmReceiptEventId] = useState<string | undefined>(
    () => dmUserId ? (room.getReadReceiptForUserId(dmUserId)?.eventId ?? undefined) : undefined
  );
  useEffect(() => {
    if (!dmUserId) return undefined;
    const handleReceipt = () => {
      const receipt = room.getReadReceiptForUserId(dmUserId);
      setDmReceiptEventId(receipt?.eventId ?? undefined);
    };
    room.on(RoomEvent.Receipt, handleReceipt);
    return () => {
      room.off(RoomEvent.Receipt, handleReceipt);
    };
  }, [room, dmUserId]);
  const dmReceiptEvent = dmReceiptEventId ? room.findEventById(dmReceiptEventId) : undefined;
  const dmReceiptTs = dmReceiptEvent?.getTs();
  ...
  const roomWallpapers = useAtomValue(roomWallpapersAtom);
  const roomWallpaper = roomWallpapers[room.roomId];
  ...
  >(
    {
      [MessageEvent.RoomMessage]: (mEventId, mEvent, item, timelineSet, collapse) => {
        ...
        const isOutgoingMsg = senderId === mx.getUserId();
        const readByOther =
          direct && isOutgoingMsg && dmReceiptTs !== undefined
            ? mEvent.getTs() <= dmReceiptTs
            : undefined;

        const msgBody = mEvent.getContent()?.body;
        const isJumboEmoji =
          messageLayout === MessageLayout.Bubble &&
          !mEvent.isRedacted() &&
          typeof msgBody === 'string' &&
          JUMBO_EMOJI_REG.test(trimReplyFromBody(msgBody));
      ...
      [MessageEvent.RoomMessageEncrypted]: (mEventId, mEvent, item, timelineSet, collapse) => {
        ...
        const isOutgoingEnc = mEvent.getSender() === mx.getUserId();
        const readByOtherEnc =
          direct && isOutgoingEnc && dmReceiptTs !== undefined
            ? mEvent.getTs() <= dmReceiptTs
            : undefined;
      ...
      [MessageEvent.Sticker]: (mEventId, mEvent, item, timelineSet, collapse) => {
        ...
        return (
          <Message
            ...
            hour24Clock={hour24Clock}
            dateFormatString={dateFormatString}
            isSticker={messageLayout === MessageLayout.Bubble}
          >
            {mEvent.isRedacted() ? (
              <RedactedContent reason={mEvent.getUnsigned().redacted_because?.content.reason} />
            ) : (
  ...
  const eventRenderer = (item: number) => {
    ...
    // Look ahead to determine if this is the last message in its group (for bubble tail)
    let isLastInGroup = true;
    if (messageLayout === MessageLayout.Bubble) {
      const nextItem = item + 1;
      const [nextTl, nextBase] = getTimelineAndBaseIndex(timeline.linkedTimelines, nextItem);
      const nextEvt = nextTl
        ? getTimelineEvent(nextTl, getTimelineRelativeIndex(nextItem, nextBase))
        : undefined;
      if (
        nextEvt &&
        !nextEvt.isRedacted() &&
        !reactionOrEditEvent(nextEvt) &&
        nextEvt.getSender() === eventSender &&
        nextEvt.getType() === mEvent.getType() &&
        minuteDifference(mEvent.getTs(), nextEvt.getTs()) < 2
      ) {
        isLastInGroup = false;
      }
    }
    ...
    const enrichedEventJSX =
      eventJSX && messageLayout === MessageLayout.Bubble && React.isValidElement(eventJSX)
        ? React.cloneElement(eventJSX as React.ReactElement<{ isLastInGroup?: boolean }>, {
            isLastInGroup,
          })
        : eventJSX;

    const newDividerJSX =
      newDivider && enrichedEventJSX && eventSender !== mx.getUserId() ? (
    ...
    if (enrichedEventJSX && (newDividerJSX || dayDividerJSX)) {
    ...
    return enrichedEventJSX;
  };

  return (
    <Box grow="Yes" style={{ position: 'relative', background: roomWallpaper ?? 'var(--tg-chat-bg, inherit)' }}>
      {unreadInfo?.readUptoEventId && !unreadInfo?.inLiveTimeline && (
        <TimelineFloat position="Top">
          ...
          >
            <Text size="L400">Отметить как прочитанное</Text>
          </Chip>
        </TimelineFloat>
      )}
      {!atBottom && (
            ...
            {room.getUnreadNotificationCount() > 0 && (
              ...
              >
                <Text as="span" size="L400">
                  {room.getUnreadNotificationCount() > 99
                    ? '99+'
                    : room.getUnreadNotificationCount()}
                </Text>
              </Badge>
            )}
            <Icon size="400" src={Icons.ArrowBottom} />
          </IconButton>
        </div>
      )}
