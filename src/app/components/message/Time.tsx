...
export const Time = as<'span', TimeProps & ComponentProps<typeof Text>>(
  ({ compact, hour24Clock, dateFormatString, ts, ...props }, ref) => {
    ...
    // Full tooltip: always show complete date + time on hover
    const fullTitle = `${timeDayMonYear(ts, dateFormatString)}, ${timeHourMinute(ts, hour24Clock)}`;

    return (
      <Text as="time" style={{ flexShrink: 0 }} size="T200" priority="300" title={fullTitle} {...props} ref={ref}>
        {time}
      </Text>
    );
  }
